...
# ==============================================================================
# ---------------------- FLASK ROUTES AND MULTI-BOT PANEL ----------------------
# (Yeh hissa panel ko connect karne ke liye joda gaya hai, upar ka logic safe hai)
# ==============================================================================
...
def add_log(msg):
    ...
    app_logs.append(f"[{time_str}] {msg}")
    if len(app_logs) > 50:
        app_logs.pop(0)
...
def api_start_bot():
    ...
    if loop:
         asyncio.run_coroutine_threadsafe(bot_task_runner(uid, password), loop)
    return jsonify({"status": "success", "message": f"Initialization started for UID {uid}"})
...
def get_logs():
    return jsonify({
        ...
        "failed": failed_bots_count
    })

@app.route('/api/active_bots', methods=['GET'])
def get_active_bots():
    bots = [{"uid": str(u)} for u in active_bot_uids]
...
# ---------------------- MAIN BOT SYSTEM (MODIFIED FOR PANEL) ----------------------

async def MaiiiinE(Uid, Pw):
    global key, iv, region, BOT_UID, active_bots_count, failed_bots_count, active_bot_uids

    add_log(f"Fetching token for UID: {Uid}...")
    open_id, access_token = await GeNeRaTeAccEss(Uid, Pw)
    if not open_id or not access_token:
        add_log(f"Error - Invalid Account or Password for {Uid}")
        failed_bots_count += 1
        return None
    ...
    if not MajoRLoGinResPonsE:
        add_log(f"Target Account {Uid} => Banned / Not Registered!")
        failed_bots_count += 1
        return None
    ...
    if not LoGinDaTa:
        add_log("Error - Getting Ports From Login Data!")
        failed_bots_count += 1
        return None
    ...
    active_bots_count += 1
    if Uid not in active_bot_uids:
        active_bot_uids.append(Uid)
        
    add_log(f"Bot Started & Online on Target: {TarGeT} | Name: {acc_name}")
    await asyncio.gather(task1, task2)

async def bot_task_runner(uid, pw):
    while True:
        try:
            await asyncio.wait_for(MaiiiinE(uid, pw), timeout=7 * 60 * 60)
        except asyncio.TimeoutError:
            add_log(f"Token Expired for {uid}, Restarting...")
        except Exception as e:
            add_log(f"Error TCP - {e} => Restarting...")
            await asyncio.sleep(5)

async def StarTinG():
    global loop
    loop = asyncio.get_running_loop()
    add_log("System Online. Waiting for panel commands...")
    while True:
